...
const lowStockCount = computed(
  () =>
    filteredProducts.value.filter(
      (p) => p.stock > 0 && p.stock <= 10 && addDaysLeft(p) > 0,
    ).length,
);
...
const expiredCount = computed(
  () =>
    filteredProducts.value.filter((p) => addDaysLeft(p) <= 0 && p.stock > 0)
      .length,
);
...
const saveProduct = () => {
  ...
  if (isEditMode.value) {
    ...
    if (index !== -1)
      products.value[index] = { ...products.value[index], ...form.value };
...
watch([searchQuery, categoryFilter, statusFilter, branchFilter], () =>
  resetPage(),
);
