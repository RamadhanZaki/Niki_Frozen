<template>
  <div class="stocks-container">
    ...
    <main class="main-content">
      <div class="top-bar">
        <div class="page-title">
          ...
          <p>Kelola stok produk dan monitoring mutasi</p>
        </div>
        <div class="top-bar-right">
          <button class="btn-add" @click="openAddStockModal">
            <svg
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card warning">
          <div class="stat-info">
            <p class="stat-label">Stok Menipis (≤10)</p>
            <p class="stat-value">{{ lowStockCount }}</p>
          </div>
        </div>
        <div class="stat-card danger">
          <div class="stat-info">
            <p class="stat-label">Stok Kritis (≤5)</p>
            <p class="stat-value">{{ criticalStockCount }}</p>
          </div>
        </div>
        <div class="stat-card info">
          <div class="stat-info">
            <p class="stat-label">Total Nilai Stok</p>
            <p class="stat-value">Rp {{ formatNumber(totalStockValue) }}</p>
          </div>
        </div>
      </div>
              ...
              >
                ...
                <td class="actions">
                  <button
                    ...
                    @click="openAdjustModal(product, 'add')"
                    title="Tambah Stok"
                  >
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  ...
                  <button
                    ...
                    @click="openAdjustModal(product, 'reduce')"
                    title="Kurangi Stok"
                  >
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      <path d="M20 12H4" />
      ...
      <div class="card" v-if="selectedProduct">
        <div class="card-header">
          <h3>Riwayat Mutasi Stok - {{ selectedProduct.name }}</h3>
          <button class="close-history" @click="selectedProduct = null">
            ✕
          </button>
        </div>
        <div class="table-responsive">
          <table class="data-table history-table">
            ...
            <tbody>
              <tr v-for="(history, idx) in stockHistory" :key="idx">
                <td>{{ history.date }}</td>
                <td>
                ...
                <td
                  :class="
                    history.type === 'in' ? 'text-success' : 'text-danger'
                  "
                >
                  {{ history.type === "in" ? "+" : "-" }} {{ history.quantity }}
                </td>
                <td>{{ history.before_stock }}</td>
                <td>{{ history.after_stock }}</td>
                <td>{{ history.note }}</td>
                <td>{{ history.user }}</td>
              </tr>
              <tr v-if="stockHistory.length === 0">
                <td colspan="7" class="empty-row">Belum ada riwayat mutasi</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            ><select v-model="stockForm.product_id" class="modal-input">
              <option value="">Pilih Produk</option>
              <option v-for="p in products" :key="p.id" :value="p.id">
                {{ p.name }} (Stok: {{ getProductStock(p.id) }})
              </option>
            </select>
          </div>
          <div class="form-group">
        ...
        <div class="modal-footer">
          ...
          ><button
            class="btn-primary"
            @click="addStock"
            :disabled="!stockForm.product_id || !stockForm.quantity"
          >
            Tambah Stok
          </button>
        </div>
    ...
    >
      <div class="modal">
        <div class="modal-header">
          <h3>{{ adjustType === "add" ? "Tambah Stok" : "Kurangi Stok" }}</h3>
        ...
        <div class="modal-body">
          <div class="product-info-card">
            ...
            <p>
              Stok Saat Ini: <strong>{{ currentProduct?.stock }} pcs</strong>
            </p>
            <p>Harga: Rp {{ formatNumber(currentProduct?.price) }}</p>
          </div>
          <div class="form-group">
            <label
              >Jumlah
              {{ adjustType === "add" ? "Ditambahkan" : "Dikurangi" }}</label
            ><input
          ...
          <div class="form-group" v-if="adjustType === 'reduce'">
            <label>Stok Setelah Pengurangan</label
            ><input
              :value="currentProduct?.stock - adjustQuantity"
              type="text"
              class="modal-input"
              disabled
            />
            <p
              v-if="currentProduct?.stock - adjustQuantity < 0"
              class="error-text"
            >
          ...
          <div class="form-group">
            ...
            ><textarea
              ...
              :placeholder="
                adjustType === 'add'
                  ? 'Contoh: Restock'
                  : 'Contoh: Kadaluarsa / Rusak'
              "
            ></textarea>
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn-secondary" @click="showAdjustModal = false">
            Batal</button
          ><button
            class="btn-primary"
            ...
            :disabled="
              !adjustQuantity ||
              (adjustType === 'reduce' &&
                currentProduct?.stock - adjustQuantity < 0)
            "
          >
            Simpan
          </button>
...
const filteredProducts = computed(() => {
  ...
  if (branchFilter.value !== "all")
    result = result.filter(
      (p) => p.branch_id.toString() === branchFilter.value,
    );
...
const openAddStockModal = () => {
  stockForm.value = { product_id: "", branch_id: 1, quantity: 0, note: "" };
